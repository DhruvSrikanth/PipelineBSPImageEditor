...
def get_parallel_times(problem_sizes, thread_nums, average_over, run_command, mode):
    ...
    for problem_size in problem_sizes:
        ...
        for thread_num in thread_nums:
            print('--------------------------------')
            for i in range(average_over):
                command = run_command.format(problem_size, mode, thread_num)
                time = subprocess.check_output(command, shell=True)
                time = str(time, encoding).strip().replace('\n', '')
...
if __name__ == '__main__':
    ...
    run_command_sequential = 'go run ../editor/editor.go {}'
    ...
    run_command_parallel = 'go run ../editor/editor.go {} {} {}'
    # For each mode, get the parallel runtimes and plot the speedups.
